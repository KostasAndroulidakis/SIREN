...
bool WebSocketMessageBroadcaster::isRunning() const noexcept {
...
void WebSocketMessageBroadcaster::broadcastMessage(const std::string& message,
                                                  const SessionContainer& sessions) {
...
uint64_t WebSocketMessageBroadcaster::getTotalBroadcasts() const noexcept {
    return total_broadcasts_.load();
}

uint64_t WebSocketMessageBroadcaster::getFailedBroadcasts() const noexcept {
    return failed_broadcasts_.load();
}

bool WebSocketMessageBroadcaster::sendToSession(std::shared_ptr<WebSocketSession> session,
...
void WebSocketMessageBroadcaster::notifyBroadcastComplete(size_t sessions_reached) {
...
void WebSocketMessageBroadcaster::updateBroadcastStats(bool success) {
